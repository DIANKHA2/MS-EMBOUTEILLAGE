...
<?php
  ...
  if(isset($_POST['submit'])){
    ...
    if($_SESSION['datedebut'] == '2023-01-27'){
      header("Location:./Arret _par_organe.php");
  }else{
    header("Location:./Arret _par_organe.php");
  }
}
   ...
   global $db;
   $stmt  = $db->prepare('CALL getArretByDistinctOrgane(?,?)');
...
<div class="bi" >  
     <div class="col-md-3 col-xs-3 spacer bn "style="margin-top:; margin-left:px; ">
    	  .<div class="panel panel-info spacer ">
    	  	<div a style= "text-align: center ; "class="panel-heading">Selectionner_un_organe</div>
    	  	  <div class="panel-body">
                     ...
                     <label class="control-label">organe:</label>
                        <select class="form-control bi" name="kla" style="width:300px; margin-left:68px;">
                               ...
                               while($et=$stmt->fetch()) { ?>
                                  <option 
                                  <?php
                                  if (!isset($_SESSION['klato'] ))
                                  {?>
                                    value="<?php  echo $et['organe'];?>"
                                 <?php  }
                                 else{
                                  if($et['organe']==$_SESSION['klato']){
                                    echo "selected";
                                  }  } 
                                  ?>
                                
                                  ><?php  echo $et['organe'];?> </option> 
                              <?php }?>
                                </optgroup>
                        </select>
                    </div>
              <button style="margin-top:10px ";type="submit" name="submit">Spécifie_un_organe</button>
             </form>
    	      </div>  	
          </div>
         </div>
