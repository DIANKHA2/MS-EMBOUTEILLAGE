...
{ $dateFin=$_POST['datefin'];
 ...
 $stmt  = $db->prepare("CALL  getbestfreqArretByMachine(:d,:f)");
 ...
 while($et=$stmt->fetch()) { 
  ...
  array_push($vales2, $et['Frequence']); 
  array_push($vales3, $et['machine']);
  }

		
 $graph = new Graph(1550,380,"auto");
...
$b1plot->SetColor("#B0C4DE");
$b1plot->SetFillColor("#B0C4DE");
$b1plot->SetLegend("Fréquence");
$b1plot->SetShadow();
 ...
 $graph->title->Set("La fréquence des arrêts par machine");
 $graph->title->SetFont(FF_GEORGIA,FS_BOLD, 20);
 $graph->xaxis->title->Set("Top 6 des machines qui posent probléme");
