...
<?php
  ...
  if(isset($_POST['submit'])){
    ...
    if($_SESSION['datedebut'] == '2023-01-27'){
      header("Location:./Arret _par_nature.php");
  }else{
    header("Location:./Arret _par_nature.php");
  }
}
   ...
   global $db;
   $stmt  = $db->prepare('CALL getArretByDistinctNature(?,?)');
...
<div class="bi" >  
     <div class="col-md-3 col-xs-3 spacer bn "style="margin-top:; margin-left:px; ">
    	  .<div class="panel panel-info spacer ">
    	  	<div a style= "text-align: center ; "class="panel-heading">Selectionner_une_nature</div>
    	  	  <div class="panel-body">
                     ...
                     <label class="control-label">nature:</label>
                        <select class="form-control bi" name="kla" style="width:300px; margin-left:68px;">
                               ...
                               while($et=$stmt->fetch()) { ?>
                                  <option 
                                  <?php
                                  if (!isset($_SESSION['klatn'] ))
                                  {?>
                                    value="<?php  echo $et['nature'];?>"
                                 <?php  }
                                 else{
                                  if($et['nature']==$_SESSION['klatn']){
                                    echo "selected";
                                  }  } 
                                  ?>
                                
                                  ><?php  echo $et['nature'];?> </option> 
                              <?php }?>
                                </optgroup>
                        </select>
                    </div>
              <button style="margin-top:10px ";type="submit" name="submit">Spécifie_une_nature</button>
             </form>
    	      </div>  	
          </div>
         </div>
