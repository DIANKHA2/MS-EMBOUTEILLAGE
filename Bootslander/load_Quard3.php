...
$a= mysqli_connect("localhost","root","","mes_soboa") or die("error connecting to the server");

$query="SELECT * FROM quard1_g1cqprod";
$res=mysqli_query($a,$query);
$vales=[];
while($et=mysqli_fetch_assoc($res)) { 
	array_push($vales, $et["quard1"]);
}
...
while($et2=mysqli_fetch_assoc($res2)) { 
	array_push($vales2, $et2["quard1"]);
}

$query3="SELECT * FROM quard1_g1cnarret";
$res3=mysqli_query($a,$query3);
$vales3=[];
while($et3=mysqli_fetch_assoc($res3)) { 
	array_push($vales3, $et3["quard1"]);
}

$query4="SELECT * FROM quard1_g1ctprod";
$res4=mysqli_query($a,$query4);
$vales4=[];
while($et4=mysqli_fetch_assoc($res4)) { 
	array_push($vales4, $et4["quard1"]);
}

$query5="SELECT * FROM quard1_g1ctrg";
$res5=mysqli_query($a,$query5);
$vales5=[];
while($et5=mysqli_fetch_assoc($res5)) { 
	array_push($vales5, $et5["quard1"]);
}
...
$b6plot->SetWidth(45);
$graph->title->Set("Analyse de données du Quard 1 : ");
